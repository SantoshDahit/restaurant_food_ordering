    ...
    <div class="container mx-auto px-4 py-10 sm:py-16">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-2xl p-6 sm:p-8 shadow-lg">
          ...
          <h3 class="text-xl font-semibold text-gray-900 mb-2">Table Ordering</h3>
          <p class="text-gray-500 text-sm mb-4">Open a table's ordering page directly by its table code.</p>
          <div v-if="!showTableInput">
            <button @click="showTableInput = true"
              class="w-full py-2 bg-blue-500 text-white rounded-xl text-sm font-medium hover:bg-blue-600 transition-colors">
              Open Table
            </button>
          </div>
          <div v-else class="space-y-2">
            ...
            <div class="flex gap-2">
              <button @click="launchTable" :disabled="!tableCode.trim()"
                class="flex-1 py-2 bg-blue-500 text-white rounded-lg text-sm font-medium hover:bg-blue-600 disabled:opacity-40 transition-colors">Go</button>
              <button @click="showTableInput = false; tableCode = ''"
                class="px-3 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm hover:bg-gray-200 transition-colors">✕</button>
            </div>
          </div>
        </div>
          ...
          <div v-else class="space-y-2">
            ...
            <div class="flex gap-2">
              <button @click="launchKiosk" :disabled="!kioskCode.trim()"
                class="flex-1 py-2 bg-teal-500 text-white rounded-lg text-sm font-medium hover:bg-teal-600 disabled:opacity-40 transition-colors">Go</button>
      ...
      <div class="mt-12 text-center">
        <router-link to="/" class="text-violet-600 hover:text-violet-700 text-sm font-medium">← Back to home</router-link>
      </div>
    </div>
  </div>
</template>
