...
const statusBorder: Record<TableStatus, string> = {
  ...
  RESERVED: 'border-info',
  CLEANING: 'border-border',
}
  ...
  <RestaurantGuard resource="tables">
    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
      <div
        ...
        :key="table.code"
        :class="['bg-card rounded-xl p-4 shadow-card border-2', statusBorder[table.status]]"
      >
        <div class="flex items-start justify-between mb-3">
          <div>
            <p class="text-xl font-bold text-foreground">#{{ table.tableNumber }}</p>
            <p class="text-xs text-muted-foreground">Cap: {{ table.capacity }}</p>
          </div>
        ...
        <button @click="launchTableMode(table.tableCode)"
          class="w-full mb-3 inline-flex items-center justify-center gap-2 px-3 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-semibold rounded-lg shadow-soft transition-all">
        ...
        <div v-if="activeOrderByTable[table.code]"
          class="w-full mb-3 bg-info/10 border border-info/20 rounded-lg p-2 space-y-2">
          <button @click="openOrder(activeOrderByTable[table.code].code)" class="w-full text-left">
            <div class="flex items-center justify-between mb-1">
              <span class="text-xs font-semibold text-info">{{ activeOrderByTable[table.code].orderNumber }}</span>
              <StatusBadge :status="activeOrderByTable[table.code].status" />
            </div>
            <div class="text-xs text-muted-foreground">
              Total <span class="font-semibold text-foreground">{{ activeOrderByTable[table.code].totalAmount.toFixed(2) }}</span>
            </div>
          </button>

          <!-- Order status -->
          <div>
            <label class="block text-[10px] font-medium text-muted-foreground uppercase tracking-wide mb-0.5">Order status</label>
            <select
              ...
              class="w-full text-xs rounded-md border border-border bg-card px-2 py-1 focus:outline-none focus:ring-2 focus:ring-ring/20 disabled:opacity-50">
          ...
          <!-- Payment status -->
          <div>
            <label class="block text-[10px] font-medium text-muted-foreground uppercase tracking-wide mb-0.5">Payment</label>
            <select v-if="paymentByOrder[activeOrderByTable[table.code].code]"
              ...
              class="w-full text-xs rounded-md border border-border bg-card px-2 py-1 focus:outline-none focus:ring-2 focus:ring-success/40 disabled:opacity-50">
            ...
            <button v-else
              ...
              class="w-full text-xs px-2 py-1 bg-success/10 ring-1 ring-success/20 text-success rounded-md hover:bg-success/20 transition-colors disabled:opacity-50">
              Mark paid (cash)
            </button>
          </div>
        </div>
        ...
        <div class="mb-3">
          <button @click="showQr(table)" class="w-full group">
            <div class="bg-muted rounded-lg p-2 flex items-center gap-2 hover:bg-accent transition-colors border border-border hover:border-primary/30">
              <svg class="w-4 h-4 text-muted-foreground group-hover:text-primary flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" />
              </svg>
              <span class="text-xs text-muted-foreground group-hover:text-primary font-medium">View QR Code</span>
            </div>
          </button>
        </div>
        ...
        <div class="flex flex-wrap gap-1">
          <button @click="openEdit(table)"
            class="text-xs px-2 py-1 bg-muted ring-1 ring-border text-foreground rounded-lg hover:bg-accent transition-colors">Edit</button>
          <button @click="deleteTarget = table.code"
            class="text-xs px-2 py-1 bg-destructive/10 ring-1 ring-destructive/20 text-destructive rounded-lg hover:bg-destructive/20 transition-colors">Del</button>
        </div>
      </div>
      <div v-if="!tables.length" class="col-span-full text-center py-12 text-muted-foreground">
        No tables found
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="showFormDialog" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-card rounded-2xl shadow-lifted ring-1 ring-border p-6 w-full max-w-sm mx-4">
          ...
          <form @submit.prevent="save" class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-foreground mb-1">Table Number</label>
      ...
      <div v-if="qrModal" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative bg-card rounded-2xl shadow-lifted p-6 w-full max-w-sm mx-4 text-center">
          <button @click="qrModal = null"
            class="absolute top-3 right-3 p-1.5 hover:bg-accent rounded-lg text-muted-foreground">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>

          <h3 class="text-lg font-bold text-foreground mb-1">Table {{ qrModal.table.tableNumber }}</h3>
          <p class="text-sm text-muted-foreground mb-4">Scan to order from this table</p>

          <!-- QR Image -->
          <div class="flex justify-center mb-4">
            <img :src="qrModal.dataUrl" alt="QR Code"
              class="w-56 h-56 rounded-xl border border-border shadow-card" />
          </div>

          <!-- URL display -->
          <div class="bg-muted rounded-xl p-3 mb-4 break-all">
            <p class="text-xs text-muted-foreground mb-1 font-medium">Customer URL</p>
          ...
          <div class="flex gap-2">
            ...
            <button @click="copyUrl"
              class="flex-1 py-2.5 bg-muted text-foreground text-sm font-medium rounded-xl hover:bg-accent transition-colors">
              Copy URL
            </button>
          </div>
        </div>
