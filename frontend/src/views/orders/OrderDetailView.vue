...
<template>
  <div>
    ...
    <div v-else-if="order" class="space-y-6">
      <!-- Order Info -->
      <div class="bg-card rounded-2xl shadow-sm ring-1 ring-border p-6">
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4">
          <div>
            <p class="text-xs text-muted-foreground">Ticket</p>
            <p v-if="order.ticketNumber != null"
              class="inline-flex items-center justify-center min-w-[3rem] px-2 py-0.5 rounded-md bg-accent text-primary ring-1 ring-primary/30 font-mono font-bold text-lg tabular-nums">
        ...
        <div v-if="order.tableNumber || order.waiterName || order.restaurantName"
          class="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4 pt-4 border-t border-border">
          <div v-if="order.restaurantName">
            <p class="text-xs text-muted-foreground">Restaurant</p>
          ...
          <div v-if="order.waiterName">
            <p class="text-xs text-muted-foreground">Waiter</p>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 pt-4 border-t border-border">
          ...
          <div>
            ...
            <p class="font-medium text-destructive">-{{ order.discountAmount.toFixed(2) }}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p class="text-lg font-bold text-foreground">{{ order.totalAmount.toFixed(2) }}</p>
          </div>
        </div>
        ...
        <div class="mt-4 pt-4 border-t border-border flex items-center gap-3">
          ...
          <select
            ...
            class="px-3 py-1.5 bg-card border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-primary/30 transition-all">
      ...
      <div class="bg-card rounded-2xl shadow-sm ring-1 ring-border p-6">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="font-semibold text-foreground">Customer Tracking</h3>
            <p class="text-xs text-muted-foreground mt-0.5">Share this link or QR so the customer can follow their order live.</p>
          </div>
        </div>
        <div class="flex flex-col sm:flex-row gap-5 items-start">
          <div class="flex-shrink-0 self-center sm:self-start">
            <div class="bg-card rounded-2xl ring-1 ring-border p-2">
              <img v-if="trackingQr" :src="trackingQr" alt="Tracking QR"
                class="w-40 h-40 sm:w-44 sm:h-44 block" />
            </div>
          </div>
          <div class="flex-1 min-w-0 w-full">
            <p class="text-xs text-muted-foreground mb-1.5 font-medium uppercase tracking-wider">Tracking URL</p>
            <div class="flex items-center gap-2 bg-muted ring-1 ring-border rounded-xl px-3 py-2">
              <code class="flex-1 min-w-0 text-xs sm:text-sm font-mono text-foreground truncate">{{ trackingUrl }}</code>
            </div>
            <div class="flex flex-wrap gap-2 mt-3">
              <button @click="copyTrackingUrl"
                class="px-4 py-2 text-sm font-medium bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl shadow-soft transition-all">
                Copy URL
              </button>
              <button @click="openTrackingPage"
                class="px-4 py-2 text-sm font-medium bg-card ring-1 ring-border text-foreground rounded-xl hover:bg-accent transition-colors">
                Open tracking page
              </button>
            </div>
            <p class="text-[11px] text-muted-foreground mt-3">Customers see the live status, a 4-step timeline, and the items list. No login required.</p>
          </div>
        </div>
      </div>

      <!-- Order Items -->
      <div class="bg-card rounded-2xl shadow-sm ring-1 ring-border">
        <div class="px-5 py-4 border-b border-border flex items-center justify-between">
          <h3 class="font-semibold text-foreground">Order Items</h3>
          <button @click="showAddItem = !showAddItem"
            class="px-3 py-1.5 text-sm bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg shadow-soft transition-all">
            + Add Item
          </button>
        </div>

        <!-- Add Item Form -->
        <div v-if="showAddItem" class="px-5 py-4 bg-muted border-b border-border">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="col-span-2">
              <label class="block text-xs font-medium text-muted-foreground mb-1">Menu Item</label>
            ...
            <div>
              <label class="block text-xs font-medium text-muted-foreground mb-1">Spice Level</label>
          ...
          <div class="flex gap-2 mt-3">
            <button @click="addItem"
              class="px-4 py-2 text-sm bg-primary text-primary-foreground rounded-lg hover:bg-primary/90">Add</button>
        ...
        <table class="w-full text-sm min-w-[640px]">
          ...
          <tbody class="divide-y divide-border">
            <tr v-for="item in orderItems" :key="item.code" class="hover:bg-accent transition-colors">
              ...
              <td class="px-5 py-3 text-muted-foreground text-xs">{{ item.notes || '—' }}</td>
              <td class="px-5 py-3 text-center">
                <button @click="removeTarget = item.code"
                  class="text-xs px-2 py-1 bg-destructive/10 ring-1 ring-destructive/20 text-destructive rounded-lg hover:bg-destructive/20 transition-colors">Remove</button>
              </td>
            </tr>
            <tr v-if="!orderItems.length">
              <td colspan="6" class="px-5 py-8 text-center text-muted-foreground">No items</td>
            </tr>
          </tbody>
        </table>
        </div>
